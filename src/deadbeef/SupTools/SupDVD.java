...
class SupDVD implements Substream, SubstreamDVD {
	 ...
	 */
	SupDVD(final String fnSup, final String fnIfo) throws CoreException {
		screenWidth = 720;
		screenHeight = 576;
		ofsXglob = 0;
		ofsYglob = 0;
	 ...
	 */
	static byte[] createSupFrame(final SubPictureDVD pic, final Bitmap bm) {
		/* create RLE buffers */
	 ...
	 * @throws CoreException
	 */
	static void writeIFO(final String fname, final SubPicture pic, final Palette pal) throws CoreException {
	...
	private void decode(final SubPictureDVD pic) throws CoreException {
		...
		bitmap  = SubDVD.decodeImage(pic, buffer, palette.getTransparentIndex());

		// crop
		final BitmapBounds bounds = bitmap.getBounds(palette, Core.getAlphaCrop());
