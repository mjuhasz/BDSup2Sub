...
public final class SubDvdWriter {
    ...
    public static byte[] createSubFrame(SubPictureDVD pic, Bitmap bm) {
        ...
        SubDvd.CONTROL_HEADER[1 + 4] = (byte) (((pic.getPal()[3] & 0xf) << 4) | (pic.getPal()[2] & 0x0f));
        SubDvd.CONTROL_HEADER[1 + 5] = (byte) (((pic.getPal()[1] & 0xf) << 4) | (pic.getPal()[0] & 0x0f));
        /* alpha (store reversed) */
        SubDvd.CONTROL_HEADER[1 + 7] = (byte) (((pic.getAlpha()[3] & 0xf) << 4) | (pic.getAlpha()[2] & 0x0f));
        SubDvd.CONTROL_HEADER[1 + 8] = (byte) (((pic.getAlpha()[1] & 0xf) << 4) | (pic.getAlpha()[0] & 0x0f));
        ...
        SubDvd.CONTROL_HEADER[1 + 17] = 0x00; /* 2 bytes subpicture size and 2 bytes control header ofs */
        SubDvd.CONTROL_HEADER[1 + 18] = 0x04; /* note: SubtitleCreator uses 6 and adds 0x0000 in between */
        ...
        int bufSize = SubDvd.PACK_HEADER.length + SubDvd.HEADER_FIRST.length + controlHeaderLen + sizeRLE;
        ...
        if (bufSize > 0x800) {
            ...
            int remainingRLEsize = sizeRLE - (0x800 - SubDvd.PACK_HEADER.length - SubDvd.HEADER_FIRST.length); // size - 0x7df
            while (remainingRLEsize > (0x800 - SubDvd.PACK_HEADER.length - SubDvd.HEADER_NEXT.length - controlHeaderLen)) {
        ...
        for (int p = 0; p < numAdditionalPackets; p++) {
            ...
            if (p == numAdditionalPackets - 1) {
                ...
                tmp = SubDvd.HEADER_NEXT.length + (controlHeaderLen - controlHeaderWritten) + (sizeRLE - ofsRLE) - 6;
            } else {
                tmp = 0x800 - SubDvd.PACK_HEADER.length - 6;
